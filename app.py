...
if authentication_status:
    ...
    def show_pdf(file_path):
        ...
        pdf_display = f'<embed src="data:application/pdf;base64,{base64_pdf}" width="1280" height="720" type="application/pdf">'
    ...
    df = pd.DataFrame({'Rapor Adı': ["Satıs Raporu - 1", "Satıs Raporu - 2", "Satıs Raporu - 3", "Stok Raporu-1", "Finans Dashboard(Satıs)",
                                     ...
                                     "Satıs Raporu - 8","Bolge Karlılık(Satıs)","E-posta Market Pazarlama(Satıs)",
                                                          "Dijital Pazarlama","Müşteri Degerlendirme-2","Yeni Müşteri","Çağrı Merkezi(Müşteri)"]},
    ...
    with st.container():
        ...
        if select_1 == "Tüm Raporlar":
            ...
            with st.container():
                ...
                st.write("---")
                st.write("**Bölge Karlılık(Satıs)**")
                show_pdf("POWERBICANLI_PDF/8_Bolge Karlılık(Satıs).pdf")

                st.write("---")
                st.write("**E-posta Market Pazarlama(Satıs)**")
                show_pdf("POWERBICANLI_PDF/9_E_posta Market Pazarlama.pdf")

                st.write("---")
                st.write("**Dijital Pazarlama**")
                show_pdf("POWERBICANLI_PDF/10_DijitalPazarlama(Satıs).pdf")

                st.write("---")
                st.write("**Müşteri Degerlendirme - 2**")
                show_pdf("POWERBICANLI_PDF/2_Client Satisfaction.pdf")

                st.write("---")
                st.write("**Yeni Müşteri**")
                show_pdf("POWERBICANLI_PDF/3_New Customer Analysis.pdf")

                st.write("---")
                st.write("**Yeni Müşteri**")
                show_pdf("POWERBICANLI_PDF/3_Call Center Insights.pdf")
        ...
        if select_1 == 'Satış Raporları':
            ...
            with col3:
                ...
                select_2 = st.selectbox('Seçim Yapınız', ['Seçim Yapınız', 'Satıs Raporu 1', 'Satıs Raporu 2', 'Satıs Raporu 3',"Finans Dashboard(Satıs)",
                                                          ...
                                                          "Satıs Raporu 6","Satıs Raporu 7","Satıs Raporu 8","Bolge Karlılık(Satıs)","E-posta Market Pazarlama(Satıs)",
                                                          "Dijital Pazarlama"
                                                          ])
    ...
    #RAPOR GÖRÜNÜMLERİ
            with st.container():
                ...
                if select_2 == "Bolge Karlılık(Satıs)":
                    with st.container():
                        st.write("---")
                        st.markdown("**BÖLGE KARLILIK(SATIS) RAPORU**")
                        components.iframe(
                            "https://app.powerbi.com/view?r=eyJrIjoiMjAwNmVhYjUtMjdhMi00OGVmLWIyMTQtY2ZhYTYxYjViNzQ2IiwidCI6IjlmZTNjZTM5LTIwOWQtNGM5NS1hMWQxLWViZjA0NjY3NDkyYyIsImMiOjl9",
                            height=600, width=1000)
                        st.write("---")
                        st.markdown("**Rapor Bilgiler**")
                        st.write("""
                        
                        Bölge Karlılık Raporu, bölgesel finansal performansa ilişkin üst düzey içgörüler sağlamak için tasarlanmış bir dizi analitik araçtır. 
                        Bu araçlar, bölgesel finansal performansın kapsamlı bir resmini oluşturmak için çapraz satış analizi, senaryo olayları ve kohort analizi gibi bir dizi veri analizi tekniği içerir. 
                        İçgörüler, kullanıcı dostu ve iş kullanıcıları için kolayca erişilebilir olan 'uygulama benzeri' bir geliştirmede sunulur.

                        Çapraz satış analizi, işletmelerin farklı ürün ve hizmetlerin belirli bir bölgedeki müşteriler tarafından nasıl birlikte kullanıldığını anlamalarına olanak tanır. 
                        Bu bilgiler, işletmelerin müşterilere çapraz satış ve ek satış için yeni fırsatları belirlemesine yardımcı olabilir. 
                        Senaryo olay analizi, işletmelerin farklı ekonomik senaryoları simüle etmesini ve bunların belirli bir bölgedeki finansal performansı nasıl etkileyebileceğini anlamasını sağlar. 
                        Kohort analizi, işletmelerin zaman içinde müşteri davranışındaki eğilimleri belirlemesine ve bu içgörülere dayalı olarak veriye dayalı kararlar almasına olanak tanır.
                        ...
                        with st.container():
                            ...
                            with col1:
                                ...
                                with open("POWERBICANLI_PDF/8_Bolge Karlılık(Satıs).pdf",
                                          "rb") as pdf_file:
                                    PDFbyte = pdf_file.read()
                                st.download_button(label="PDF Görünümü İndir", key='3',
                                                   data=PDFbyte,
                ...
                if select_2 == "E-posta Market Pazarlama(Satıs)":
                    with st.container():
                        st.write("---")
                        st.markdown("**E-posta Market Pazarlama(Satıs)**")
                        components.iframe(
                            "https://app.powerbi.com/view?r=eyJrIjoiOWM2MmY4M2EtYmI2Yi00Y2Q4LThhYzYtZTkzMmNlMThkZmZlIiwidCI6IjlmZTNjZTM5LTIwOWQtNGM5NS1hMWQxLWViZjA0NjY3NDkyYyIsImMiOjl9",
                            height=600, width=1000)
                        st.write("---")
                        st.markdown("**Rapor Bilgiler**")
                        st.write("""

                        E-posta pazarlama stratejinizin etkinliğini daha iyi anlamak için satış ve müşteri bağlılığı arasındaki ilişkiyi analiz etmek önemlidir. 
                        İşletmeler ve kuruluşlar, bu iki ölçümün birbiriyle nasıl ilişkili olduğunu anlayarak, müşteri katılımını artırmak ve daha fazla satış sağlamak için e-posta pazarlama kampanyalarını optimize edebilir.

                        Bu analiz sayesinde işletmeler ve kuruluşlar, hangi tür e-postaların müşteri katılımını ve satışları artırmada en etkili olduğunu ve ayrıca hangi müşteri segmentlerinin e-posta pazarlamasına en çok yanıt verdiğini belirleyebilir. 
                        Bu bilgilerle donanan işletmeler ve kuruluşlar, hedef kitlelerinde yankı uyandırma ve dönüşümleri artırma olasılığı daha yüksek olan hedefli e-posta pazarlama kampanyaları geliştirebilir.
                        ...
                        with st.container():
                            ...
                            with col1:
                                ...
                                with open("POWERBICANLI_PDF/9_E_posta Market Pazarlama.pdf",
                                          "rb") as pdf_file:
                                    PDFbyte = pdf_file.read()
                                st.download_button(label="PDF Görünümü İndir", key='3',
                                                   data=PDFbyte,
                ...
                if select_2 == "Dijital Pazarlama":
                    with st.container():
                        st.write("---")
                        st.markdown("**Dijital Pazarlama**")
                        components.iframe(
                            "https://app.powerbi.com/view?r=eyJrIjoiZGM2NTFjZmItMDgyNS00NTI5LTk5NjYtNzk3MzVmODIwMDgyIiwidCI6IjlmZTNjZTM5LTIwOWQtNGM5NS1hMWQxLWViZjA0NjY3NDkyYyIsImMiOjl9",
                            height=600, width=1000)
                        st.write("---")
                        st.markdown("**Rapor Bilgiler**")
                        st.write("""

                            Pazarlama girişimlerinizin etkinliğini daha iyi anlamak için kampanyalarınızın zaman içindeki performansını karşılaştıran bir analiz yapmak önemlidir. 
                            Tıklama oranları, dönüşüm oranları ve katılım seviyeleri gibi temel ölçümleri inceleyerek, pazarlama çalışmalarınızın etkinliğine ilişkin değerli bilgiler edinebilirsiniz.



                            Bu analiz, hangi kampanyaların hedef kitlenizde en fazla ilgi ve etkileşim oluşturduğunu ve hangilerinin aynı performansı göstermediğini belirlemenize yardımcı olabilir. 
                            Bu bilgileri kullanarak, pazarlama bütçenizi nereye tahsis edeceğiniz konusunda veriye dayalı kararlar alabilir, mesajlaşma ve hedefleme stratejilerinizi ayarlayabilir ve daha iyi sonuçlar elde etmek için kampanyalarınızı optimize edebilirsiniz. 
                            Bu yaklaşım, pazarlama hedeflerinize daha verimli ve etkili bir şekilde ulaşmanıza yardımcı olabilir, bu da artan yatırım getirisi ve iyileştirilmiş iş sonuçları sağlar.
                        ...
                        with st.container():
                            ...
                            with col1:
                                ...
                                with open("POWERBICANLI_PDF/10_DijitalPazarlama(Satıs).pdf",
                                          "rb") as pdf_file:
                                    PDFbyte = pdf_file.read()
                                st.download_button(label="PDF Görünümü İndir", key='3',
                                                   data=PDFbyte,
    ...
    Pazarlama analitiği, işletmelere daha iyi pazarlama kararları alabilme, müşterilerle daha etkili iletişim kurabilme ve rekabet avantajı elde etme imkanı sunar. Verilere dayalı pazarlama stratejileri sayesinde işletmeler, hedef kitleye daha etkili şekilde ulaşabilir, müşteri memnuniyetini artırabilir ve satış performansını iyileştirebilir.
        ...
        if select_1 == 'Müşteri Analitiği':
            ...
            with col3:
                st.write("**Rapor Seçimi**")
                select_2 = st.selectbox('Lütfen Görmek İstediğiniz Raporu Seçiniz', ['Seçim Yapınız', "Müşteri Analitigi 1", "Müşteri Analitigi 2","Müşteri Değerlendirme","Müşteri Degerlendirme-2","Yeni Müşteri","Çağrı Merkezi(Müşteri)"])
                        ...
                        with st.container():
                            ...
                            with col1:
                                ...
                                st.download_button(label="PDF Görünümü İndir", key='3',
                                                   ...
                                                   file_name="Rapor Görünümü.pdf",
                                                   mime='application/octet-stream')
                if select_2 == "Müşteri Degerlendirme-2":
                    with st.container():
                        st.write("---")
                        st.markdown("**Müşteri Değerlendirme RAPORU 2**")
                        components.iframe(
                            "https://app.powerbi.com/view?r=eyJrIjoiZmQ2ODEyNDktNzAzOS00N2VlLTg5MzQtZmI2Yjg5YmE2MmEzIiwidCI6IjlmZTNjZTM5LTIwOWQtNGM5NS1hMWQxLWViZjA0NjY3NDkyYyIsImMiOjl9",
                            height=600, width=1000)
                        st.write("---")
                        st.markdown("**MÜŞTERİ ANALİTİĞİ**")
                        # with st.expander("Raporun Hedef Kitlesi Kimdir?"):
                        ...
                        Müşteri şikayetlerini etkili bir şekilde yönetmek için, en yaygın şikayet kategorileri, şikayetleri çözmek için harcanan ortalama süre ve çözüm sunması daha uzun süren çalışanlar gibi çeşitli ölçümleri takip etmek çok önemlidir. 
                        İşletmeler, bu ölçümleri izleyerek iyileştirme gerektiren alanları belirleyebilir ve müşteri memnuniyetini artırmak için uygun önlemleri alabilir.



                        İzlenmesi gereken önemli bir ölçüt, en fazla şikayeti oluşturan şikayet kategorisidir. 
                        İşletmeler, ortak kategorileri anlayarak, şikayetlerin temel nedenini ele almak için önlemler alabilir ve böylece şikayetlerin sıklığını azaltabilir. 
                        Ek olarak, şikayetleri çözmek için harcanan sürenin izlenmesi, işletmelerin şikayetleri ele alma süreçlerindeki darboğazları belirlemesine ve süreci kolaylaştırmak için adımlar atmasına yardımcı olabilir. 
                        Son olarak, çözüm sunması daha uzun süren çalışanlara göz kulak olmak, işletmelerin eğitim ihtiyaçlarını belirlemesine ve buna ihtiyacı olan çalışanlara ek destek sağlamasına yardımcı olarak müşteri hizmetlerinin genel kalitesini artırabilir. """)
                        ...
                        with st.container():
                            ...
                            with col1:
                                ...
                                with open("POWERBICANLI_PDF/2_Client Satisfaction.pdf",
                                          "rb") as pdf_file:
                                    PDFbyte = pdf_file.read()
                                st.download_button(label="PDF Görünümü İndir", key='3',
                                                   data=PDFbyte,
                                                   file_name="Rapor Görünümü.pdf",
                                                   mime='application/octet-stream')
                if select_2 == "Yeni Müşteri":
                    with st.container():
                        st.write("---")
                        st.markdown("**Yeni Müşteri RAPORU**")
                        components.iframe(
                            "https://app.powerbi.com/view?r=eyJrIjoiZGEzNGQyYjEtOGMwYy00M2I2LTkyNmUtYWZiMGE4MjRjMDgxIiwidCI6IjlmZTNjZTM5LTIwOWQtNGM5NS1hMWQxLWViZjA0NjY3NDkyYyIsImMiOjl9",
                            height=600, width=1000)
                        st.write("---")
                        st.markdown("**MÜŞTERİ ANALİTİĞİ**")
                        # with st.expander("Raporun Hedef Kitlesi Kimdir?"):
                        ...
                        Etkili bir müşteri hizmeti oluşturmak, ekibin büyük çaba sarf etmesini gerektiren oldukça çok yönlü bir süreçtir. 
                        Müşteri memnuniyeti oldukça öznel olduğundan, müşteri onayı almakla memnuniyetsizlik arasında ince bir çizgi vardır. 
                        Zorluk aynı zamanda artan müşteri sayısıyla birlikte gelir ve tüm geri bildirimlerin oluşturulmasını oldukça kapsamlı hale getirir. 
                        Bununla birlikte, müşteri davranışının eğilimlerini ve modellerini gösteren verilerden yararlanırken, yüksek kalite standardını korumak hala mümkündür.
                        
                        
                        Power BI ile Yeni Müşteri Analizi Raporu, bu verilerin kullanılmasına ve analiz edilmesine olanak tanır. 
                        Bu rapor, 
                            Müşteri hizmetinizin mevcut durumunu ve bunun müşteri davranışı üzerindeki etkilerini belirlemenize,
                            Trendleri nasıl tahmin edeceğinizi ve olasılıkları nasıl tahmin edeceğinizi görebilmenizi, 
                            Beklenen olayları nasıl ele alabileceğinizi ve sizi olumlu sonuçlara nasıl yönlendirebileceğiniz konusunda yardımcı olacaktır. 
                        Tüm bunlar, Power BI'ın analitik özelliklerinden yararlanılarak yapılır.
                        ...
                        with st.container():
                            ...
                            with col1:
                                ...
                                with open("POWERBICANLI_PDF/3_New Customer Analysis.pdf",
                                          "rb") as pdf_file:
                                    PDFbyte = pdf_file.read()
                                st.download_button(label="PDF Görünümü İndir", key='3',
                                                   data=PDFbyte,
                                                   file_name="Rapor Görünümü.pdf",
                                                   mime='application/octet-stream')
                if select_2 == "Çağrı Merkezi(Müşteri)":
                    with st.container():
                        st.write("---")
                        st.markdown("**Çağrı Merkezi Takip Raporu**")
                        components.iframe(
                            "https://app.powerbi.com/view?r=eyJrIjoiNzgxZjA4MDMtMDE4My00MjIxLThhYzEtNzNiNjRkMTRjMTlmIiwidCI6IjlmZTNjZTM5LTIwOWQtNGM5NS1hMWQxLWViZjA0NjY3NDkyYyIsImMiOjl9",
                            height=600, width=1000)
                        st.write("---")
                        st.markdown("**MÜŞTERİ ANALİTİĞİ**")
                        # with st.expander("Raporun Hedef Kitlesi Kimdir?"):
                        ...
                        Bir çağrı merkezinin performansını değerlendirmek için müşteri memnuniyetini ve operasyonel verimliliği etkileyen temel ölçütlerin analizini yapmak önemlidir. 
                        Bu değerlendirme, terk etme oranı, memnuniyet puanı, hizmet düzeyi anlaşması (SLA) ve ortalama yanıt hızı gibi metriklerin incelenmesini içerecektir. 
                        Kuruluşlar, bu ölçütleri inceleyerek çağrı merkezi operasyonlarının etkinliğine ilişkin içgörüler elde edebilir ve iyileştirilecek alanları belirleyebilir.
                        
                        
                        
                        Terk etme oranını analiz ederek, kaç kişinin bir müşteri temsilcisiyle konuşmadan önce telefonu kapattığını belirlemek mümkündür; bu, arama yönlendirme veya bekleme süreleriyle ilgili sorunları gösterebilir. 
                        Memnuniyet puanlarının incelenmesi, temsilciler tarafından sağlanan müşteri hizmetlerinin kalitesini ölçmeye ve iyileştirme alanlarını belirlemeye yardımcı olabilir. 
                        Benzer şekilde, SLA'nın izlenmesi, aramaların kabul edilebilir zaman dilimlerinde yanıtlanmasını sağlamaya yardımcı olabilir ve ortalama yanıt hızının analizi, çağrı merkezi operasyonlarının verimliliğine ilişkin içgörüler sağlayabilir. 
                        Kuruluşlar, bu metriklerden yararlanarak müşteri memnuniyetini artırmak, üretkenliği artırmak ve daha iyi iş sonuçları elde etmek için çağrı merkezi operasyonlarını optimize edebilir.
                        ...
                        with st.container():
                            ...
                            with col1:
                                ...
                                with open("POWERBICANLI_PDF/3_Call Center Insights.pdf",
                                          "rb") as pdf_file:
                                    PDFbyte = pdf_file.read()
                                st.download_button(label="PDF Görünümü İndir", key='3',
                                                   data=PDFbyte,
